...
<li class="nav-item">
    <a href="view2.php?id=<?php echo urlencode($id); ?>&loginName=<?php echo urlencode($loginName)?>&church=<?php echo urlencode($church)?>&contact=<?php echo urlencode($contact); ?>">
...
if (isset($_GET['date']) && isset($_GET['church'])) {
    ...
    // Select the record with the given date from the "money" table
    $select = "SELECT * FROM money WHERE date=?";
...
if (isset($_POST['update_btn'])) {
    ...
    $update = "UPDATE money SET amount=? WHERE date=?";
    ...
    if ($result) {
        ...
        $id = $row['id'];
        $contact = $_GET['contact']; // Add this line to retrieve the "contact" parameter

        echo '<script type="text/javascript">
            alert("Data Updated Successfully");
            window.location.href = "view2.php?loginName=' . $loginName . '&id=' . $id . '&church=' . $church . '&contact=' . $contact . '";
        </script>';
        exit();
    } else {
...
<script>
    function sendData() {
        ...
        var url = "view2.php?date=" + encodeURIComponent(date) + "&church=" + encodeURIComponent(church) + "&loginName=" + encodeURIComponent(loginName) + "&id=" + encodeURIComponent(id)+ "&contact=" + encodeURIComponent(contact);
