...
<?php
    ...
    if (isset($_GET['church'])) {
        ...
        $church = mysqli_real_escape_string($conn, $church);

        // Query to retrieve rows that match the church
        $query = "SELECT id, firstname, middlename, lastname, dob, contact FROM registration WHERE church = '$church'";
        ...
        if ($result) {
            ...
            // Print button
            echo "<button onclick='printTable(\"$church\")'>Print</button>";
        } else {
