...
?>
<li class="nav-item">
    <a href="view3.php?id=<?php echo urlencode($id); ?>&loginName=<?php echo urlencode($loginName)?>&church=<?php echo urlencode($church)?>&contact=<?php echo urlencode($contact); ?>">
...
// Check if the date and church parameters exist in the URL
if (isset($_GET['date']) && isset($_GET['church'])) {
    $date = $_GET['date'];
    $church = $_GET['church'];

    // Select the record with the given date and church from the "mdf" table
    $select = "SELECT * FROM annual WHERE date='$date' AND church='$church'";
...
if (isset($_POST['update_btn'])) {
    ...
    // Update the record with the given date and church in the "mdf" table
    $update = "UPDATE annual SET amount='$amount' WHERE date='$date' AND church='$church'";
    ...
    if ($result) {
        ...
        header("Location: view3.php?loginName=$loginName&id=$id&church=$church&contact=$contact");
        exit();
    } else {
...
<script>
    function sendData() {
        ...
        var url = "view3.php?date=" + encodeURIComponent(date) + "&church=" + encodeURIComponent(church) + "&loginName=" + encodeURIComponent(loginName) + "&id=" + encodeURIComponent(id)+ "&contact=" + encodeURIComponent(contact);
