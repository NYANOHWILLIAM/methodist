...
// Query to get the total number of adherents
$sqlAdherents = "SELECT 
    COUNT(*) AS adherent_count,
    SUM(CASE WHEN gender = 'Male' THEN 1 ELSE 0 END) AS male_adherent_count,
    SUM(CASE WHEN gender = 'Female' THEN 1 ELSE 0 END) AS female_adherent_count
FROM registration WHERE membership = 'adherent' AND church = ?";
$stmtAdherents = $conn->prepare($sqlAdherents);

if ($stmtAdherents === false) {
    die("Preparation of Adherents query failed: " . $conn->error);
}

$stmtAdherents->bind_param("s", $churchFromURL);
$stmtAdherents->execute();
$resultAdherents = $stmtAdherents->get_result();

if ($resultAdherents && $resultAdherents->num_rows > 0) {
    $rowAdherents = $resultAdherents->fetch_assoc();
    $adherentCount = $rowAdherents['adherent_count'];
    $maleAdherentCount = $rowAdherents['male_adherent_count'];
    $femaleAdherentCount = $rowAdherents['female_adherent_count'];
    
    echo "ADHERENTS<br>";
    echo "Total number of adherents: " . $adherentCount . "<br>";
    echo "Total number of male adherents: " . $maleAdherentCount . "<br>";
    echo "Total number of female adherents: " . $femaleAdherentCount . "<br>";
} else {
    echo "No data found for the specified church or no adherents.";
}
